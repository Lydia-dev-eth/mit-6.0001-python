...
def img_to_pix(filename):
    ...
    pixels=imge.get_flattened_data()
    pixel_tuple=list(pixels)
    return pixel_tuple
...
def filter(pixels_list, color):
    ...
    if color==None:
        return pixels_list
    matriix=make_matrix(color)
    list_pixel=[]
    for pixel in pixels_list:
        cahnged_pixel= matrix_multiply(matriix,pixel)

        new_pixel=[]
        for flot in cahnged_pixel:
            new_pixel.append(int(flot))
        list_pixel.append(tuple(new_pixel))  
    return new_pixel
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if isinstance(pixel, tuple):
        list1=[]
        for int1 in pixel:
           end_bit=int1 % 2**num_end_bits
           list1.append(end_bit)
        return(tuple(list1))
...
def reveal_bw_image(filename):
    ...
    """
    img1 = Image.open("hidden1.bmp")
    list_pixels= img_to_pix(filename)
    ...
    for p in list_pixels:
        extracted = extract_end_bits( num_bits,p)
        new_pixels.append(int(extracted * scale))

       

    img =  pix_to_img(new_pixels,img1.size,"L")
...
def reveal_color_image(filename):
    ...
    Returns:
        result: an Image object containing the hidden image
    """
    img1 = Image.open("hidden2.bmp")
    list_pixels= img_to_pix(filename)
    num_bits =3
    scale = 255 / (2**num_bits - 1)
    new_pixels = []
    for tupl in list_pixels:
        list1=[] # to hold the  new values of each int in the tuple 
        for int1 in tupl:
            extracted = extract_end_bits( num_bits,int1)
            list1.append(int(extracted * scale))
        new_pixels.append(tuple(list1))
    img =  pix_to_img(new_pixels,img1.size,'RGB')
    return img
